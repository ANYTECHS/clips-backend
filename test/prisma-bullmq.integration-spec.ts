/**
 * Prisma + BullMQ integration smoke — verifies enqueue + updateClip path.
 */
...
describe('Prisma + BullMQ integration', () => {
  it('enqueues a clip-generation job and returns a jobId', async () => {
    ...
    const service = new ClipsService(
      ...
      {
        enqueue: jest.fn(async ({ queue: q, jobName, data }: any) => {
    ...
    await service.updateClip(1, { clipUrl: 'https://cdn.example.com/clip.mp4', status: 'success' });
